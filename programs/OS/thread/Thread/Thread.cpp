...
bool CThread::Create(){
    ...
    bool ret = true;
#ifdef WIN32
    m_hThread = ::CreateThread(NULL,9,_ThreadEntry,this,0,&m_IDThread);
    if(m_hThread == NULL){
        ret = false;
    }
#else
    ret = (::pthread_create(&m_hThread,NULLm&_ThreadEntry,this) == 0);
#endif
    return ret;
}
...
bool CThread::Join(){
    THREAD_HANDLE m_hThread = GetHandle();
    if(m_hThread == (THREAD_HANDLE)0){
        return true;
    }
#ifdef WIN32
...
#else
    return (::pthread_join(hTread,NULL) == 0);
#endif
}
